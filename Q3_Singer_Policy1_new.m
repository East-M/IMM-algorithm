...
for num = 1:Num
    ...
    for k = 1:N
        ...
        Q_singer_adaptive = scale_Q * Q_singer;     % 调节过程噪声矩阵

        Pk_estm = F_singer * Pk * F_singer' + Q_singer_adaptive;                % 预测
        K_singer = Pk_estm * H_singer' / (H_singer * Pk_estm * H_singer' + R_singer);   % 卡尔曼增益
        Pk = (I - K_singer * H_singer) * Pk_estm;                           % 更新

        X_pre = KalmanFilter(Z(:,k), X_pre, F_singer, H_singer, K_singer);
...
if vis == true
    ...
    exportgraphics(gcf, "策略一：Singer+kappa+tau滤波结果.pdf", "ContentType", "vector")
    ...
    plot(zv_filted);
    legend("True Speed","Result");
    exportgraphics(gcf, "策略一：Singer+kappa+tau模型的速度的滤波结果.pdf", "ContentType", "vector")
    ...
    plot(z_filted);
    legend("Real Trajectory","Observed Data","Result");
    exportgraphics(gcf, "策略一：Singer+kappa+tau模型的位置的滤波结果.pdf", "ContentType", "vector")
