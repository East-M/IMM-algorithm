...
T = 0.5;         % 扫描周期
N = 389;         % 观测点个数
sigma = 1000;           % 过程噪声方差(为设计参数，需要反复调试)
...
F_ca_augmented = blkdiag(F_ca, eye(2));             % 假设曲率和挠率随机游走

% 系统扰动矩阵：
G_ca = blkdiag([T^3/6;T^2/2;T],[T^3/6;T^2/2;T],[T^3/6;T^2/2;T]);
G_ca_augmented = blkdiag(G_ca, eye(2));             % 曲率和挠率随机游走下，过程噪声扩展

% 量测矩阵
% H_ca = blkdiag([1,0,0],[1,0,0],[1,0,0]);
% H_ca_augmented = [H_ca, zeros(3,2)];                % 仅观测位置
H_ca_augmented = eye(11);

% 过程噪声协方差矩阵
Q_ca = sigma^2 * (G_ca * G_ca');
Q_ca_augmented = blkdiag(Q_ca, diag([sigma_kappa^2, sigma_tau^2])); % 新增噪声项
...
for num = 1:Num
    ...
    for k = 1:N
        ...
        Pk_estm = F_ca_augmented * Pk * F_ca_augmented' + Q_ca_augmented;                % 预测
        K_ca = Pk_estm * H_ca_augmented' / (H_ca_augmented * Pk_estm * H_ca_augmented' + R_ca);   % 卡尔曼增益
        Pk = (I - K_ca * H_ca_augmented) * Pk_estm;                           % 更新

        X_pre = KalmanFilter(Z(:,k), X_pre, F_ca_augmented, H_ca_augmented, K_ca);
...
if vis == true
    ...
    exportgraphics(gcf, "策略二：CA+kappa+tau滤波结果.pdf", "ContentType", "vector")
    ...
    plot(zv_filted);
    legend("True Speed","Result");
    exportgraphics(gcf, "策略二：CA+kappa+tau模型的速度的滤波结果.pdf", "ContentType", "vector")
    ...
    plot(z_filted);
    legend("Real Trajectory","Observed Data","Result");
    exportgraphics(gcf, "策略二：CA+kappa+tau模型的位置的滤波结果.pdf", "ContentType", "vector")
