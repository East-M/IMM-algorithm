...
% 采用策略一：根据曲率kappa和扰率tau  动态调整Q_cv。
...
P0 = diag([sigma_r^2, 50^2, 0, sigma_r^2, 50^2, 0, sigma_r^2, 50^2, 0]); % 设置初始位置，初始速度，初始加速度的估计（滤波）误差方差
...
% 系统扰动矩阵： CV模型下，加速度扰动影响位置和速度的方式遵循牛顿运动定律
...
%% 蒙特卡洛仿真实验
X_cv_filted = zeros(xNum,N);                    % 初始化滤波观测值
rmse_obs = 0;                                   % 初始化观测值的rmse
...
for num = 1:Num
    ...
    for k = 1:N
        ...
        Q_cv_adaptive = scale_Q * Q_cv;             % 调节过程噪声矩阵

        Pk_estm = F_cv * Pk * F_cv' + Q_cv_adaptive;                % 预测
        K_cv = Pk_estm * H_cv' / (H_cv * Pk_estm * H_cv' + R_cv);   % 卡尔曼增益
        Pk = (I - K_cv * H_cv) * Pk_estm;                           % 更新

        X_pre = KalmanFilter(Z(:,k), X_pre, F_cv, H_cv, K_cv);
...
if vis == true
    ...
    exportgraphics(gcf, "策略一：CV+kappa+tau滤波结果.pdf", "ContentType", "vector")
    ...
    plot(zv_filted);
    legend("True Speed","Result");
    exportgraphics(gcf, "策略一：CV+kappa+tau模型的速度的滤波结果.pdf", "ContentType", "vector")
    ...
    plot(z_filted);
    legend("Real Trajectory","Observed Data","Result");
    exportgraphics(gcf, "策略一：CV+kappa+tau模型的位置的滤波结果.pdf", "ContentType", "vector")
