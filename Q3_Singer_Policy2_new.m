...
% 解决问题三：基于Singer模型，结合曲率挠率对数据进行滤波，并做可视化处理。
...
xNum = 11;          % 状态矢量参数量
zNum = 11;          % 观测矢量参数量
P0 = diag([sigma_r^2, 50^2, 25^2, sigma_r^2, 50^2, 25^2, sigma_r^2, 50^2, 25^2, 100^2, 100^2]);
...
F_singer_augmented = blkdiag(F_singer, eye(2));             % 假设曲率和挠率随机游走

% 量测矩阵
% H_singer = blkdiag([1,0,0],[1,0,0],[1,0,0]);
H_singer_augmented = eye(11);
...
Q_singer_augmented = blkdiag(Q_singer, diag([sigma_kappa^2, sigma_tau^2]));
...
for num = 1:Num
    ...
    ax = XA_t + normrnd(0, sigma_r, size(XV_t));
    ay = YA_t + normrnd(0, sigma_r, size(YV_t));
    az = ZA_t + normrnd(0, sigma_r, size(ZV_t));
    ...
    for k = 1:N
        ...
        if k >= 3
            v = [X_singer_filted(2,k-1), X_singer_filted(5,k-1), X_singer_filted(8,k-1)]; % 速度向量
            a = [(X_singer_filted(2,k-1) - X_singer_filted(2,k-2)) / dt, 
                (X_singer_filted(5,k-1) - X_singer_filted(5,k-2)) / dt,
                (X_singer_filted(8,k-1) - X_singer_filted(8,k-2)) / dt];  % 加速度向量
            j = [(X_singer_filted(3,k-1) - X_singer_filted(3,k-2)) / dt, 
                (X_singer_filted(6,k-1) - X_singer_filted(6,k-2)) / dt,
                (X_singer_filted(9,k-1) - X_singer_filted(9,k-2)) / dt];  % 加加速度向量
        else
            v = [X_pre(2), X_pre(5), X_pre(8)];     % 速度向量,用pre设置可能有点问题，不过都是小问题，后面都会调回来
            a = [X_pre(3), X_pre(6), X_pre(9)];     % 加速度向量,用pre设置可能有点问题，不过都是小问题，后面都会调回来
            j = [0,0,0];
        end
        [kappa, tau] = kappa_tau_cal(v,a,j);        % 计算得到此时的曲率和扰率，由于是cv模型，加速度始终是0，所以tau始终是0

        Pk_estm = F_singer_augmented * Pk * F_singer_augmented' + Q_singer_augmented;                % 预测
        K_singer = Pk_estm * H_singer_augmented' / (H_singer_augmented * Pk_estm * H_singer_augmented' + R_singer);   % 卡尔曼增益
        Pk = (I - K_singer * H_singer_augmented) * Pk_estm;                           % 更新

        X_pre = KalmanFilter(Z(:,k), X_pre, F_singer_augmented, H_singer_augmented, K_singer);
...
if vis == true
    ...
    title("Singer with kappa and tau");
    xlabel("x(m)"); ylabel("y(m)"); zlabel("z(m)");
    grid on; hold on;
    plot3(x_filted, y_filted, z_filted, 'LineWidth', 2);
    plot3(X_t, Y_t, Z_t, 'green', 'LineWidth', 2);
    legend("Observed Data", "Result", "Real Trajectory");
    exportgraphics(gcf, "策略二：Singer+kappa+tau滤波结果.pdf", "ContentType", "vector")
    ...
    plot(zv_filted);
    legend("True Speed","Result");
    exportgraphics(gcf, "策略二：Singer+kappa+tau模型的速度的滤波结果.pdf", "ContentType", "vector")
    ...
    plot(z_filted);
    legend("Real Trajectory","Observed Data","Result");
    exportgraphics(gcf, "策略二：Singer+kappa+tau模型的位置的滤波结果.pdf", "ContentType", "vector")
