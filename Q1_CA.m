...
if exist(K_ca_Path,"file") == 0
    % 未找到离线计算的卡尔曼增益文件，进行计算和保存
    K_ca = KalmanGain(P0, F_ca, H_ca, Q_ca, R_ca);
    save(K_ca_Path, "K_ca");
    disp("已离线计算卡尔曼增益! 保存于【"+K_ca_Path+"】");
else
    load(K_ca_Path);
    disp("已加载CA模型的卡尔曼增益!");
end
...
for num = 1:Num
    ...
    tic;
    for k = 1:N
        X_pre = KalmanFilter(Z(:,k), X_pre, F_ca, H_ca, K_ca(:,:,k));
